...
// Takes a value of the Area enum as input and return's the area's name as a std::string.
std::string areaToString(Area input);

// Displays the input std::string with proper wrapping, i.e. no split words and no spaces at the start of a wrapped line.  This function WILL print out spaces after a newline character.
// If width is specified, the input is printed out with that width, not the set screen width.
void display(std::string input, uint width = 0);
...
std::string truncateText(std::string name, int length, uint quantity = 1);
